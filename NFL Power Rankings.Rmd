...
First, we need to load the required packages for this project.
...
Next, we will use the NFL games data from Lee Sharpe. We can connect to his raw file on github and use the `readRDS()` function to load it in memory.
...
Outliers heavily impact the performance of this approach. Feel free to trim down the points of certain games. Or if you use the play-by-play data provided by the `{nflfastR}` package, you can filter out garage time points. 
